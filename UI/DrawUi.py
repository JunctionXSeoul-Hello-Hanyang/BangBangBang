    ...
    def draw_total(self):
        ...
        for current in self.board.boardSection:
            ...
            if current.boardLocation.sectionNumber == 38:
                ...
                current.card = Card("blank_card",0,0,0,0)
                self.draw_one(current.card, current.boardLocation, condition)
                current.card = Card("background",0,0,0,0)


            self.draw_one(current.card,current.boardLocation,condition)
        pygame.display.update()
        right_card = self.board.boardSection[37].card
        self.board = BoardSection.BoardSection([self.current_player] + self.other_player_list)
        # right image가 Yourturn일 경우 유지한다.
        if right_card.name == 'yourTurn' and self.board.boardSection[37]:
            self.board.boardSection[37].card = right_card
